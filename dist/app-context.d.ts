...
export interface SamEmailIngestService {
    ...
    fetchAttachment(msg: unknown, attachmentId: string): Promise<{
        bytes: Buffer;
        name: string;
        contentType: string;
    }>;
    getAttachmentText(msg: unknown, attachmentId: string, opts?: {
        maxBytes?: number;
    }): Promise<{
        name: string;
        contentType: string;
...
export type AppBackendFactory = (context: AppContext) => import('express').Router | Promise<import('express').Router>;
