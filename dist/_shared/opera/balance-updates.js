 ...
 */
export async function updateNbankBalance(trx, bankAccount, amountPounds) {
    ...
    if (Number(rows) === 0) {
        throw new Error(`nbank balance update failed: bank account '${bankAccount}' not found ` +
...
const nacntTypeCache = new WeakMap();
export async function getNacntType(trx, account) {
    ...
    const rows = (await trx.raw(`SELECT na_type, na_subt FROM nacnt WITH (NOLOCK)
     WHERE RTRIM(na_acnt) = ?`, [key]));
 ...
 */
export async function updateNacntBalance(trx, account, value, opts) {
    const period = Number(opts.period);
    const year = Number(opts.year);
    if (!VALID_PERIODS.has(period))
        return; // skip — same as Python warning+return
    ...
    const rows = await trx('nacnt')
        ...
        .update(nacntUpdate);
    if (Number(rows) === 0) {
        throw new Error(`nacnt update affected 0 rows for account ${account_} - ` +
...
async function updateNhist(trx, account, value, period, year) {
    const typeInfo = await getNacntType(trx, account);
    if (!typeInfo)
        return; // matches Python's warning + return — non-fatal
    const costCentre = '    ';
    const findRows = (await trx.raw(`SELECT TOP 1 id FROM nhist WITH (UPDLOCK, ROWLOCK)
     WHERE RTRIM(nh_nacnt) = ?
       AND nh_ntype = ?
       AND nh_nsubt = ?
       AND nh_ncntr = ?
       AND nh_year = ?
       AND nh_period = ?`, [account, typeInfo.na_type, typeInfo.na_subt, costCentre, year, period]));
    const id = Array.isArray(findRows) && findRows[0] ? Number(findRows[0].id) : null;
    if (id !== null && Number.isFinite(id)) {
        if (value >= 0) {
            await trx.raw(`UPDATE nhist WITH (ROWLOCK)
        ...
        await trx.raw(`INSERT INTO nhist (
         id,
 ...
 */
export async function insertNjmemo(trx, journalNumber, description) {
    const { getNextId } = await import('./id-allocation.js');
    ...
    await trx.raw(`INSERT INTO njmemo (
...
async function updateNsubtNtype(trx, account, value) {
    const typeInfo = await getNacntType(trx, account);
    if (!typeInfo)
        return; // matches Python warning + return
    // nsubt
    await trx.raw(`UPDATE nsubt WITH (ROWLOCK)
     SET ns_balance = ISNULL(ns_balance, 0) + ?,
         datemodified = GETDATE()
     WHERE ns_subt = ? AND ns_type = ?`, [value, typeInfo.na_subt, typeInfo.na_type]);
    // ntype
    await trx.raw(`UPDATE ntype WITH (ROWLOCK)
     SET nt_bal = ISNULL(nt_bal, 0) + ?,
         datemodified = GETDATE()
     WHERE nt_type = ?`, [value, typeInfo.na_type]);
}
//# sourceMappingURL=balance-updates.js.map