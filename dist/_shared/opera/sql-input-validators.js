...
export function validateBankCode(value) {
    ...
    if (!BANK_CODE_RE.test(value)) {
        throw new SqlInputValidationError(`bank_code '${value}' is not a valid Opera bank code ` +
            '(alphanumeric/underscore/dash, max 12 chars).');
    }
    const bad = hasForbidden(value);
    if (bad) {
        throw new SqlInputValidationError(`bank_code contains forbidden character '${bad}'.`);
    }
    return value.trim();
}
export function validateAccountCode(value) {
    ...
    if (!ACCOUNT_CODE_RE.test(value)) {
        throw new SqlInputValidationError(`account code '${value}' is not valid ` +
            '(alphanumeric/_/./-/, max 16 chars).');
    }
    const bad = hasForbidden(value);
    if (bad) {
        throw new SqlInputValidationError(`account code contains forbidden character '${bad}'.`);
    }
    return value.trim();
}
export function validateEntryNumber(value) {
    if (value === null || value === undefined || value === '') {
        throw new SqlInputValidationError('entry number required');
    }
    if (!ENTRY_NUMBER_RE.test(value)) {
        throw new SqlInputValidationError(`entry number '${value}' is not a valid Opera entry reference.`);
    }
    const bad = hasForbidden(value);
    if (bad) {
        throw new SqlInputValidationError(`entry number contains forbidden character '${bad}'.`);
    }
    return value.trim();
}
export function validateCbtype(value) {
    if (!value)
        return '';
    if (!CBTYPE_RE.test(value)) {
        throw new SqlInputValidationError(`cbtype '${value}' is not valid (max 4 alphanumeric chars).`);
    }
    return value.trim().toUpperCase();
}
export function validatePaymentRef(value) {
    if (!value) {
        throw new SqlInputValidationError('payment_ref required');
    }
    if (!PAYMENT_REF_RE.test(value)) {
        throw new SqlInputValidationError(`payment_ref '${value}' contains invalid characters.`);
    }
    const bad = hasForbidden(value);
    if (bad) {
        throw new SqlInputValidationError(`payment_ref contains forbidden character '${bad}'.`);
    }
    return value.trim();
}
export function validateReference(value) {
    ...
    if (!REFERENCE_RE.test(s)) {
        throw new SqlInputValidationError(`reference '${s}' contains invalid characters.`);
    }
    const bad = hasForbidden(s);
    if (bad) {
        throw new SqlInputValidationError(`reference contains forbidden character '${bad}'.`);
    }
    return s.trim();
}
export function validateBatchNumber(value) {
    const s = String(value);
    if (!BATCH_NUMBER_RE.test(s)) {
        throw new SqlInputValidationError(`batch number '${value}' must be digits only (max 9 chars).`);
    }
    return parseInt(s, 10);
}
//# sourceMappingURL=sql-input-validators.js.map