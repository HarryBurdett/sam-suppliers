 ...
 */
export async function getAgedDebtSummary(operaDb) {
    try {
        const sql = `
      SELECT
        CASE
    ...
    `;
        const rows = (await operaDb.raw(sql));
        const buckets = (Array.isArray(rows) ? rows : []).map((r) => ({
            age_band: r.age_band ?? '',
 ...
 */
export async function getAgedDebtBySupplier(operaDb) {
    try {
        const sql = `
      SELECT
    ...
    `;
        const rows = (await operaDb.raw(sql));
        const suppliers = (Array.isArray(rows) ? rows : []).map((r) => ({
...
//# sourceMappingURL=aged-debt.js.map