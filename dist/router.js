...
import { getGlobalSupplierSettings, updateGlobalSupplierSettings, } from './services/global-settings.js';
...
import { getAgedCreditorsSummary, getAgedCreditorsTrend, getAgedCreditorsDetail, } from './services/aged-creditors.js';
import { getMergedContacts, createOperaContact, updateOperaContact, deleteOperaContact, } from './services/opera-contacts.js';
import { listContacts, addContact, deleteContact, } from './services/contacts.js';
import { listApprovedEmails, approveEmail, revokeEmail, } from './services/approved-emails.js';
import { getSupplierConfig, saveSupplierConfig, } from './services/supplier-config.js';
import { listStatements, getStatement, } from './services/supplier-statements.js';
import { getAutomationConfig, saveAutomationConfig, } from './services/automation-config.js';
import { getOnboardingState, listOnboardingStates, updateOnboardingState, } from './services/onboarding.js';
import { listRemittanceLog, recordRemittance, } from './services/remittance-log.js';
import { listCommunications, recordCommunication, deleteCommunication, } from './services/communications.js';
import { listChangeAudit, recordChange } from './services/change-audit.js';
import { listStatementLines, addStatementLines, updateStatementLineMatch, deleteStatementLines, listOperaOnlyItems, } from './services/statement-lines.js';
import { isEmailProcessed, recordProcessedEmail, listProcessedEmails, } from './services/processed-emails.js';
import { listOverrides, recordOverride, deleteOverride, } from './services/supplier-overrides.js';
import { getStatementQueue, getStatementsDashboard, getStatementHistory, } from './services/statement-queue.js';
import { listQueries, resolveQuery, autoResolveQueries, listOverdueQueries, recordReminderSent, } from './services/supplier-queries.js';
import { processStatement, acknowledgeStatement, approveStatement, editStatementResponse, bulkApproveStatements, } from './services/statement-actions.js';
import { listSecurityAlerts, verifySecurityAlert, listSecurityAuditLog, scanSupplierChanges, } from './services/security.js';
import { getCreditorsDashboard, getCreditorsReport, searchCreditors, getCreditorsSupplier, getCreditorsSupplierTransactions, getStatementPdf, previewStatementResponse, sendUpdatedStatementStatus, extractStatementFromText, processStatementEmail, reconcileStatementByEmail, listReconciliations, getFlaggedEmails, getSupplierAccountByCode, getFirstSupplierAccount, } from './services/misc-endpoints.js';
...
export function createRouter(ctx) {
    ...
    router.post('/api/suppliers/settings', async (req, res) => {
        ...
        try {
            const body = (req.body ?? {});
            const result = await updateGlobalSupplierSettings(appDb, body);
            if (!result.success) {
    ...
    router.get('/api/suppliers/directory', async (req, res) => {
        ...
        try {
            ...
            const result = await listSupplierDirectory(operaDb, ctx.db.app ?? null, { search });
            res.json(result);
        }
        catch (err) {
    ...
    router.get('/api/creditors/aged/:account', async (req, res) => {
        ...
        if (!operaDb)
            return;
        try {
            const result = await getAgedCreditorsDetail(operaDb, String(req.params.account ?? ''));
            if (!result.success) {
    ...
    router.get('/api/supplier-contacts/:account', async (req, res) => {
        ...
        if (!operaDb)
            return;
        try {
            const result = await getMergedContacts(operaDb, ctx.db.app, String(req.params.account ?? ''));
            res.json(result);
        }
        catch (err) {
    ...
    router.post('/api/supplier-contacts/:account/opera', async (req, res) => {
        ...
        try {
            const body = (req.body ?? {});
            const result = await createOperaContact(operaDb, ctx.db.app, String(req.params.account ?? ''), body);
            if (!result.success) {
                res.status(400).json(result);
                return;
            }
    ...
    router.put('/api/supplier-contacts/:account/opera/:contact_id', async (req, res) => {
        ...
        try {
            ...
            const body = (req.body ?? {});
            const result = await updateOperaContact(operaDb, ctx.db.app, String(req.params.account ?? ''), contactId, body);
            if (!result.success) {
    ...
    router.delete('/api/supplier-contacts/:account/opera/:contact_id', async (req, res) => {
        ...
        try {
            const contactId = Number(req.params.contact_id);
            const result = await deleteOperaContact(operaDb, ctx.db.app, String(req.params.account ?? ''), contactId);
            if (!result.success) {
                const status = /not found/i.test(result.error ?? '') ? 404 : 400;
                res.status(status).json(result);
                return;
    ...
    router.get('/api/suppliers/statements', async (req, res) => {
        ...
        if (!appDb)
            return;
        try {
            const supplierCode = typeof req.query.supplier_code === 'string' ? req.query.supplier_code : null;
    ...
    router.put('/api/suppliers/:code/automation', async (req, res) => {
        ...
        try {
            ...
            const result = await saveAutomationConfig(appDb, {
                supplier_code: code,
                auto_process: typeof body.auto_process === 'boolean' ? body.auto_process : undefined,
    ...
    router.get('/api/suppliers/remittance', async (req, res) => {
        ...
        if (!appDb)
            return;
        try {
            const supplierCode = typeof req.query.supplier_code === 'string'
    ...
    router.get('/api/suppliers/communications', async (req, res) => {
        ...
        try {
            const result = await listCommunications(appDb, {
                supplierCode: typeof req.query.supplier_code === 'string'
                    ? req.query.supplier_code
                    : null,
                channel: typeof req.query.channel === 'string'
                    ? req.query.channel
                    : null,
                fromDate: typeof req.query.from_date === 'string'
                    ? req.query.from_date
                    : null,
                toDate: typeof req.query.to_date === 'string' ? req.query.to_date : null,
        ...
        try {
            const result = await listChangeAudit(appDb, {
                supplierCode: typeof req.query.supplier_code === 'string'
                    ? req.query.supplier_code
                    : null,
                changedField: typeof req.query.changed_field === 'string'
                    ? req.query.changed_field
                    : null,
                fromDate: typeof req.query.from_date === 'string' ? req.query.from_date : null,
                toDate: typeof req.query.to_date === 'string' ? req.query.to_date : null,
                limit: req.query.limit ? Number(req.query.limit) : undefined,
            });
            if (!result.success) {
     ...
     */
    router.get('/api/suppliers/processed-emails/:message_id/exists', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    // ---------------------------------------------------------------
    router.get('/api/supplier-statements/queue', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    router.get('/api/supplier-statements/dashboard', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    router.get('/api/supplier-statements/history', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
            const supplierCode = req.query.supplier_code || null;
    ...
    // Supplier queries
    // ---------------------------------------------------------------
    router.get('/api/supplier-queries', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    router.post('/api/supplier-queries/:query_id/resolve', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    router.post('/api/supplier-queries/auto-resolve', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        ...
        try {
            const lookup = {
                hasMatchingPosting: async ({ supplierCode, reference, amountPounds, }) => {
                    try {
            ...
            const body = (req.body ?? {});
            const result = await autoResolveQueries(appDb, lookup, body.resolved_by ?? 'system');
            res.json(result);
        }
        catch (err) {
    ...
    router.get('/api/supplier-queries/overdue', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    function buildSupplierLookup(operaDb) {
        return {
    ...
    function buildPtranLookup(operaDb) {
        return {
            forSupplier: async (code) => {
                try {
                    const rows = (await operaDb('ptran')
                        ...
                        .select('pt_unique', 'pt_trref', 'pt_supref', 'pt_trtype', 'pt_trvalue', 'pt_trbal', 'pt_trdate'));
                    return rows;
                }
                catch {
                    return [];
    ...
    router.post('/api/supplier-statements/:id/process', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        ...
        try {
            ...
            const result = await processStatement(appDb, id, buildPtranLookup(operaDb));
            res.json(result);
        }
        catch (err) {
    ...
    router.post('/api/supplier-statements/:id/acknowledge', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        ...
        try {
            const id = Number(req.params.id);
            const result = await acknowledgeStatement(appDb, buildEmailSender(), buildSupplierLookup(operaDb), id);
    ...
    router.post('/api/supplier-statements/:id/approve', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        ...
        try {
            const id = Number(req.params.id);
            const body = (req.body ?? {});
            const result = await approveStatement(appDb, buildEmailSender(), buildSupplierLookup(operaDb), id, {
    ...
    router.put('/api/supplier-statements/:id/response', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    router.post('/api/supplier-statements/queue/bulk-approve', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        ...
        try {
            ...
            const result = await bulkApproveStatements(appDb, buildEmailSender(), buildSupplierLookup(operaDb), {
    ...
    // ---------------------------------------------------------------
    function buildPnameProvider(operaDb) {
        return {
            snapshot: async () => {
                try {
                    const rows = (await operaDb('pname').select(operaDb.raw('RTRIM(pn_account) AS account'), operaDb.raw('RTRIM(pn_name) AS name'), operaDb.raw("RTRIM(ISNULL(pn_bankac, '')) AS pn_bankac"), operaDb.raw("RTRIM(ISNULL(pn_banksor, '')) AS pn_banksor"), operaDb.raw("RTRIM(ISNULL(pn_email, '')) AS pn_email")));
                    return rows ?? [];
                }
                catch {
                    return [];
            ...
            resolveNames: async (codes) => {
                ...
                try {
                    const rows = (await operaDb('pname')
                        ...
                        .select(operaDb.raw('RTRIM(pn_account) AS code'), operaDb.raw('RTRIM(pn_name) AS name')));
                    const out = {};
                    for (const r of rows) {
                        out[r.code] = r.name;
                    }
    ...
    router.get('/api/supplier-security/alerts', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
            const result = await listSecurityAlerts(appDb, buildPnameProvider(operaDb));
            res.json(result);
        }
        catch (err) {
    ...
    router.post('/api/supplier-security/alerts/:id/verify', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
            const id = Number(req.params.id);
            const body = (req.body ?? {});
            const result = await verifySecurityAlert(appDb, id, body.verified_by ?? 'System');
            if (!result.success) {
                res.status(404).json(result);
                return;
            }
    ...
    router.get('/api/supplier-security/audit', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        ...
        try {
            ...
            const result = await listSecurityAuditLog(appDb, buildPnameProvider(operaDb), days);
            res.json(result);
        }
        catch (err) {
    ...
    router.post('/api/supplier-security/scan-changes', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
            const result = await scanSupplierChanges(appDb, buildPnameProvider(operaDb), buildSecurityEmail());
            res.json(result);
        }
        catch (err) {
    ...
    // ---------------------------------------------------------------
    router.get('/api/supplier-directory', async (req, res) => {
        req.url = '/api/suppliers/directory';
        router.handle(req, res, () => undefined);
    });
    router.get('/api/supplier-settings', async (req, res) => {
        req.url = '/api/suppliers/settings';
        router.handle(req, res, () => undefined);
    });
    router.post('/api/supplier-settings', async (req, res) => {
        req.url = '/api/suppliers/settings';
        router.handle(req, res, () => undefined);
    });
    router.get('/api/supplier-config', async (req, res) => {
        // Forward to /api/suppliers (list of supplier_config rows)
        req.url = '/api/suppliers';
        router.handle(req, res, () => undefined);
    });
    router.post('/api/supplier-config/sync', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const operaDb = getOperaDb(req, res);
        ...
        try {
            ...
            const rows = (await operaDb.raw(`SELECT RTRIM(pn_account) AS account, RTRIM(pn_name) AS name
    ...
    router.get('/api/supplier-config/:account', async (req, res) => {
    ...
    router.put('/api/supplier-config/:account', async (req, res) => {
        const account = String(req.params.account ?? '');
        req.url = `/api/suppliers/${encodeURIComponent(account)}/config`;
        router.handle(req, res, () => undefined);
    });
    router.get('/api/supplier-config/:account/detail', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const account = String(req.params.account ?? '');
    ...
    router.get('/api/supplier-communications', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    router.get('/api/supplier-communications/:account', async (req, res) => {
        const account = String(req.params.account ?? '');
    ...
    // ---------------------------------------------------------------
    router.get('/api/supplier-statements', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    router.get('/api/supplier-statements/reconciliations', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await listReconciliations(appDb));
    });
    router.get('/api/supplier-statements/:statement_id', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
    ...
    router.get('/api/supplier-statements/:statement_id/lines', async (req, res) => {
    ...
    router.get('/api/supplier-statements/:statement_id/pdf', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await getStatementPdf(appDb, Number(req.params.statement_id)));
    });
    ...
    router.post('/api/supplier-statements/:statement_id/process', async (req, res) => {
        const id = String(req.params.statement_id ?? '');
    ...
    router.post('/api/supplier-statements/:statement_id/acknowledge', async (req, res) => {
        const id = String(req.params.statement_id ?? '');
        req.url = `/api/supplier-statements/${encodeURIComponent(id)}/acknowledge`;
        req.params.id = id;
        router.handle(req, res, () => undefined);
    });
    router.post('/api/supplier-statements/:statement_id/approve', async (req, res) => {
        const id = String(req.params.statement_id ?? '');
        req.url = `/api/supplier-statements/${encodeURIComponent(id)}/approve`;
        req.params.id = id;
        router.handle(req, res, () => undefined);
    });
    router.put('/api/supplier-statements/:statement_id/edit-response', async (req, res) => {
        const id = String(req.params.statement_id ?? '');
        req.url = `/api/supplier-statements/${encodeURIComponent(id)}/response`;
        req.params.id = id;
        router.handle(req, res, () => undefined);
    });
    router.post('/api/supplier-statements/:statement_id/preview-response', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const llm = ctx.llm ?? null;
        res.json(await previewStatementResponse(appDb, llm, Number(req.params.statement_id)));
    });
    router.post('/api/supplier-statements/:statement_id/send-updated-status', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const body = (req.body ?? {});
        res.json(await sendUpdatedStatementStatus(appDb, Number(req.params.statement_id), String(body.status ?? 'received'), String(body.updated_by ?? 'system')));
    });
    router.post('/api/supplier-statements/extract-from-text', async (req, res) => {
        const llm = ctx.llm ?? null;
        const body = (req.body ?? {});
        res.json(await extractStatementFromText(llm, String(body.content ?? '')));
    });
    router.post('/api/supplier-statements/extract-from-file', async (req, res) => {
        ...
        res.status(501).json({
            success: false,
            error: 'Multipart file upload not implemented in this plugin port. POST text content under "content" or use /extract-from-text.',
        });
    });
    router.post('/api/supplier-statements/extract-from-email/:email_id', async (req, res) => {
        const llm = ctx.llm ?? null;
        const attachments = ctx.supplierEmailAttachments ?? builtinEmailIngest?.attachments;
        if (!attachments) {
            res.status(503).json({
                success: false,
    ...
    router.post('/api/supplier-statements/process-email/:email_id', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await processStatementEmail(appDb, Number(req.params.email_id)));
    });
    router.post('/api/supplier-statements/reconcile/:email_id', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await reconcileStatementByEmail(appDb, Number(req.params.email_id)));
    });
    // ---------------------------------------------------------------
    // /api/supplier-security/* aliases + email-flags
    // ---------------------------------------------------------------
    router.post('/api/supplier-security/alerts/:alert_id/verify', async (req, res) => {
        const id = String(req.params.alert_id ?? '');
        req.url = `/api/supplier-security/alerts/${encodeURIComponent(id)}/verify`;
        req.params.id = id;
        router.handle(req, res, () => undefined);
    });
    router.get('/api/supplier-security/email-flags', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await getFlaggedEmails(appDb));
    });
    router.get('/api/supplier-security/approved-senders', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    router.post('/api/supplier-security/approved-senders', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const body = (req.body ?? {});
    ...
    router.delete('/api/supplier-security/approved-senders/:sender_id', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
    ...
    // ---------------------------------------------------------------
    router.get('/api/creditors/dashboard', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        res.json(await getCreditorsDashboard(operaDb));
    });
    router.get('/api/creditors/report', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        res.json(await getCreditorsReport(operaDb));
    });
    router.get('/api/creditors/search', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        res.json(await searchCreditors(operaDb, String(req.query.q ?? '')));
    });
    router.get('/api/creditors/supplier/:account', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        res.json(await getCreditorsSupplier(operaDb, String(req.params.account ?? '')));
    });
    router.get('/api/creditors/supplier/:account/transactions', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        res.json(await getCreditorsSupplierTransactions(operaDb, String(req.params.account ?? '')));
    });
    router.get('/api/creditors/supplier/:account/statement', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const supplier = await getCreditorsSupplier(operaDb, String(req.params.account ?? ''));
        const txns = await getCreditorsSupplierTransactions(operaDb, String(req.params.account ?? ''));
        res.json({
    ...
    // ---------------------------------------------------------------
    router.get('/api/supplier/account/first', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        res.json(await getFirstSupplierAccount(operaDb));
    });
    router.get('/api/supplier/account/:account', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        res.json(await getSupplierAccountByCode(operaDb, String(req.params.account ?? '')));
    });
